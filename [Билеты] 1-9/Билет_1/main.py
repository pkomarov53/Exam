    ...
    if file_path:
        try:
            ...
            df = pd.DataFrame([line.strip().split(',') for line in lines],
                              columns=['дата', 'номер зачетки', 'ФИО', 'группа', 'дисциплина', 'оценка'])
...
def add_record():
    global df
    if df is not None:
        date = askstring("Введите дату", "Введите дату (гггг-мм-дд):")
        ...
        grade = askstring("Введите оценку", "Введите оценку:")
